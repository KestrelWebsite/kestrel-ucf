...
export interface MemberCardProps {
  ...
  major?: string;
  contribution: string;
  image: string; // string instead of StaticImageData
...
const MemberCard = ({ name, role, major, contribution, image, links }: MemberCardProps) => {
  return (
    <div className="group relative rounded-xl border border-white/10 bg-white/5 backdrop-blur-md shadow-md hover:shadow-lg transition-all duration-300 p-6 flex flex-col justify-between">
      ...
      <div>
        ...
        {major && (
          <p className="text-sm text-gray-500 italic mt-1">{major}</p>
        )}
      </div>
      ...
      {/* Image (optional placeholder handling) */}
